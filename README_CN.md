...
登录网页后切换到“频道库”标签，提交当前 Telegram 账号可访问的频道或超级群消息链接，即可建立可恢复的完整历史包索引。链接只用于识别会话；重复提交同一会话会打开已有频道库，不会重复扫描一份数据。

- 首次全量扫描固定每批 50 个消息 ID，批间等待 4-6 秒；耗时取决于最大消息 ID 和历史缺口，没有固定完成时长。
- 扫描在 Telegram 下载排队或进行时自动让行，下载空闲后继续；也可在批次边界手动暂停、继续或停止。
- 扫描期间可浏览稳定包。首次全量到达 `ready` 或 `partial` 后，才能提交下载。
- 可按标题、发布时间、消息 ID、媒体数量、已知总大小和下载状态筛选；“全选筛选结果”跨分页生效，选择在刷新后保留。
