...
* 资源 Bot 的“下载 → 暂存频道 → 转发到用户频道”发布链路已停用；资源包查询和下载可通过 Web 或 Hermes MCP 控制
...
Hermes 可通过本机 MCP `stdio` 服务跨机器查询资源包、提交显式下载、查看和控制任务，并管理关键词监控。部署、Bearer Key 鉴权、工具清单和停用发布链路说明见 [`docs/mcp-control.md`](docs/mcp-control.md)。
...
| Hermes MCP 控制 | 通过 `stdio` 查询资源包、提交下载、查看/控制任务和管理关键词监控 |
