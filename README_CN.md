...
“资源包”标签是跨频道聚合入口，可直接搜索、筛选和选择多个频道的资源包，来源频道是每个包的关联元数据和可选多选筛选条件。“频道”标签只负责提交频道链接、查看扫描状态、维护索引和展示当前频道的资源统计及监控关键词分布。重复提交同一会话不会重复扫描一份数据。
...
- 首次全量结束后可手动发起增量扫描，也可通过一个全局 cron 定时检查全部频道。没有新增消息时不会创建空扫描任务；频道存在全量、增量、补扫、暂停或停止任务时，本轮自动检查跳过该频道且不积压。增量和补扫固定每批 50 个消息 ID，批间等待 1-2 秒。
...
- **channel_library.incremental_scan_cron** - 所有已索引频道共用的 5 段 cron 表达式；默认空字符串表示关闭。修改后需要重启。
- **channel_library.incremental_scan_timezone** - cron 使用的 IANA 时区，默认 `Asia/Shanghai`。
## 执行
